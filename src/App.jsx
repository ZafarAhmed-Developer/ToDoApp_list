...
function App() {
  ...
  let saveToDoList= (event)=>{
    ...
    if(!toname) return;
    let isDuplicate = todoList.some(item=>item.toLowerCase()===toname.toLowerCase());
    if(!isDuplicate)
    {
  ...
  let list =todoList.map((item, index)=>{
    return(
      <TodoListItem value= {item} key={index} indexNumber = {index} todoList={todoList}
  ...
  return (
   <div className="App  mt-20">
    ...
    <form onSubmit= {saveToDoList}>
      <input  type='text' name='toname' className='border border-black rounded-md'/> 
      <button className='border border-black bg-blue-500 text-white rounded-md'>save</button>
    </form>
